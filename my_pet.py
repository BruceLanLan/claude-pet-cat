...
def display_pet(pet, frame=0):
    ...
    W = 44  # 内容区宽度
    print("╔" + "═" * W + "╗")
    print(f"║ 🏠 宠物之家  💰 1000  📅 {pet.get_age_str():>8} ║")
    print("╠" + "═" * W + "╣")
    print("║" + " " * W + "║")
    ...
    print("║" + " " * W + "║")
    thought_line = f'  💭 "{pet.thought}"'
    print(f"║{thought_line:<{W}}║")
    print("╠" + "═" * W + "╣")
    ...
    for icon, label, val in [("🍖", "饱食", pet.hunger), ("💖", "快乐", pet.happiness),
                              ("⚡", "能量", pet.energy), ("❤️", "健康", pet.health),
                              ("💕", "爱意", pet.love)]:
        line = f" {icon} {label}: [{bar(val)}] {val:3}%"
        print(f"║{line:<{W}}║")

    print("╠" + "═" * W + "╣")
    stage_names = {"baby": "幼年期", "child": "成长期", "adult": "成熟期", "perfect": "完全体", "ultimate": "究极体"}
    stage = stage_names.get(pet.evolution_stage, "幼年期")
    info_line = f" Lv.{pet.level} {pet.name} {element_ico} {stage}"
    print(f"║{info_line:<{W}}║")
    dna_line = f" DNA: HP:{pet.dna['hp']:3} ATK:{pet.dna['attack']:2} DEF:{pet.dna['defense']:2} SPD:{pet.dna['speed']:2} LCK:{pet.dna['luck']:2}"
    print(f"║{dna_line:<{W}}║")
    # 显示技能
    print("╠" + "═" * W + "╣")
    print(f"║{' 🎯 技能:':<{W}}║")
    for i, skill_id in enumerate(pet.skills[:2]):
        skill = SKILLS_DATA.get(skill_id, {})
        sk_line = f"    {i+1}. {skill.get('name', skill_id):8} PP:{pet.skill_pp.get(skill_id, 0):2}/{skill.get('pp', 0):2}"
        print(f"║{sk_line:<{W}}║")
    print("╚" + "═" * W + "╝")
